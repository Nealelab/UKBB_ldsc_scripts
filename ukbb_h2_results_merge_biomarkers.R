...
# setwd("~/Documents/Code/round2_h2")
...
phenbio <- read.delim("biomarkers.both_sexes.tsv.bgz",header=T,stringsAsFactors=F,sep='\t')
phenbio$sex <- "both_sexes"
phenbio_male <- read.delim("biomarkers.male.tsv.bgz",header=T,stringsAsFactors=F,sep='\t')
phenbio_male$sex <- "male"
phenbio_female <- read.delim("biomarkers.female.tsv.bgz",header=T,stringsAsFactors=F,sep='\t')
...
######
# load download of icd10, finngen, covariate, biomarker h2 results from google bucket (no updates)
...
# gsutil cp gs://ukb31063-ldsc-results/h2part_misc/ukbb31063.{both_sexes,female,male}.h2part_results.biomarkers.batch_1.tsv.gz ./
# gsutil cp gs://ukb31063-ldsc-results/h2part_misc/ukbb31063.{both_sexes,female,male}.h2part_results.biomarkers.dilution.batch_1.tsv.gz ./
######
...
dat_bio <- read.delim("ukbb31063.both_sexes.h2part_results.biomarkers.batch_1.tsv.gz",header=T,stringsAsFactors=F,sep='\t',quote="")
dat_bio$sex <- "both_sexes"
dat_bio_female <- read.delim("ukbb31063.female.h2part_results.biomarkers.batch_1.tsv.gz",header=T,stringsAsFactors=F,sep='\t',quote="")
dat_bio_female$sex <- "female"
dat_bio_male <- read.delim("ukbb31063.male.h2part_results.biomarkers.batch_1.tsv.gz",header=T,stringsAsFactors=F,sep='\t',quote="")
...
dat_biod <- read.delim("ukbb31063.both_sexes.h2part_results.biomarkers.dilution.batch_1.tsv.gz",header=T,stringsAsFactors=F,sep='\t',quote="")
dat_biod$sex <- "both_sexes"
dat_biod_female <- read.delim("ukbb31063.female.h2part_results.biomarkers.dilution.batch_1.tsv.gz",header=T,stringsAsFactors=F,sep='\t',quote="")
dat_biod_female$sex <- "female"
dat_biod_male <- read.delim("ukbb31063.male.h2part_results.biomarkers.dilution.batch_1.tsv.gz",header=T,stringsAsFactors=F,sep='\t',quote="")
...
##### quick biomarker check:

biom <- h2[!is.na(h2$dilute),]
...
png(file="~/Downloads/biomaker_dilution_h2.png",width=8,height=8,units="in",res=300)
par(mfrow=c(2,2))
plot(biodat$h2_liability.x,biodat$h2_liability.y,
     main="h2 estimate",
     xlab="gwas covariates only",ylab="add dilution factor",
     col=c("black","blue","orange")[as.numeric(factor(biodat$sex,levels = c("both_sexes","female","male")))],
     pch=c(1,20)[1+grepl("_irnt",biodat$phenotype)],
     cex=1.5)
legend("bottomright",legend=c("both_sexes","female","male","irnt","raw"),col=c("black","blue","orange",NULL,NULL),lty=c(1,1,1,NA,NA),lwd=c(5,5,5,1,1),pch=c(NA,NA,NA,20,1))
abline(0,1,col="red",lwd=2)
plot(biodat$h2_z.x,biodat$h2_z.y,
     main="h2 z score",
     xlab="gwas covariates only",ylab="add dilution factor",
     col=c("black","blue","orange")[as.numeric(factor(biodat$sex,levels = c("both_sexes","female","male")))],
     pch=c(1,20)[1+grepl("_irnt",biodat$phenotype)],
...
legend("bottomright",legend=c("both_sexes","female","male","irnt","raw"),col=c("black","blue","orange",NULL,NULL),lty=c(1,1,1,NA,NA),lwd=c(5,5,5,1,1),pch=c(NA,NA,NA,20,1))
abline(0,1,col="red",lwd=2)
plot(biodat$intercept.x,biodat$intercept.y,
...
legend("bottomright",legend=c("both_sexes","female","male","irnt","raw"),col=c("black","blue","orange",NULL,NULL),lty=c(1,1,1,NA,NA),lwd=c(5,5,5,1,1),pch=c(NA,NA,NA,20,1))
abline(0,1,col="red",lwd=2)
plot(biodat$intercept_z.x,biodat$intercept_z.y,
     main="intercept z score",
     xlab="gwas covariates only",ylab="add dilution factor",
     col=c("black","blue","orange")[as.numeric(factor(biodat$sex,levels = c("both_sexes","female","male")))],
     pch=c(1,20)[1+grepl("_irnt",biodat$phenotype)],
     cex=1.5)
...
con <- gzfile("ukb31063.h2.baseline1-1.gwas_v2.full_results.14Aug2019.tsv.gz","w")
