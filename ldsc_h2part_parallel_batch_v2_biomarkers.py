...
phen_summary = 'gs://ukb-mega-gwas-results/round2/annotations/biomarkers.{}.tsv.bgz'

ss_bucket = 'gs://ukb-mega-gwas-results/round2/additive-ldsc-sumstats/biomarkers'
...
parser.add_argument('--sex-group', type=str, required=True, help="results for male, female, or both_sexes")
parser.add_argument('--phsource', type=str, required=True, help="phenotype set (e.g. phesant, icd10, finngen)")
...
# extract args
sex_group = args.sex_group
phsource = args.phsource
num_phens = int(args.numphens)
phen_summary = phen_summary.format(sex_group)

# compute parallel split
idx = xrange(args.paridx-1, num_phens, args.parsplit)
# h2_fname = 'ukbb31063.'+str(sex_group)+'.h2part_results.'+str(phsource)+'.batch_'+str(args.paridx)+'.tsv.gz' #output file name
h2_fname = 'ukbb31063.'+str(sex_group)+'.h2part_results.'+str(phsource)+'.dilution.batch_'+str(args.paridx)+'.tsv.gz' #output file name
...
if not os.path.isdir('/home/mtag/mtag-master/ld_ref_panel/1000G_Phase3_weights_hm3_no_MHC/'):
...
def process_h2_part(h2_results, outfile, phname, phdesc, phsource, n, ncas=None, ncon=None):
...
def ldsc_h2_part(args, **kwargs):
    ...
    # ss_name = str(phname)+'.imputed_v3.ldsc.'+sex_group+'.tsv.gz'
    ss_name = str(phname)+'.imputed_v3.ldsc.dilution_factor.'+sex_group+'.tsv.gz'
...
if __name__ == "__main__":
    ...
    phens_source = phens.loc[phens['source'] == phsource, :]
    print('idx: ' + str(idx))
    ph_sub = phens_source.iloc[idx]
    ph_list = list(ph_sub.phenotype.values)
    ...
    iter_args = itertools.izip(ph_list, ph_sub['description'], ph_sub['source'], ph_sub['n_non_missing'], ph_sub['n_cases'], ph_sub['n_controls'])

    # bake in globals
    ldsc_h2_map = partial(ldsc_h2_part, wd=wd, ld_ref_panel=ld_ref_panel, ld_w_panel=ld_w_panel, ld_frq_panel=ld_frq_panel, ss_bucket=ss_bucket, sex_group=str(sex_group))
