...
# Biomarker dilution factor {#dilution}
...
# Continuous phenotype normalization {#irnt}
...
# Redundant FinnGen codes {#finngen}
...
con1 <- gzfile(paste0(params$outdir,"/bothsexes_h2_topline_temp.tsv.gz"),"w")
write.table(dat_full,file=con1,sep='\t',col.names=T,row.names=F,quote=F)
close(con1)

con2 <- gzfile(paste0(params$outdir,"/female_h2_topline_temp.tsv.gz"),"w")
write.table(datf_full,file=con2,sep='\t',col.names=T,row.names=F,quote=F)
close(con2)

con3 <- gzfile(paste0(params$outdir,"/male_h2_topline_temp.tsv.gz"),"w")
write.table(datm_full,file=con3,sep='\t',col.names=T,row.names=F,quote=F)
close(con3)

con4 <- gzfile(paste0(params$outdir,"/h2_topline_temp.tsv.gz"),"w")
write.table(dat,file=con4,sep='\t',col.names=T,row.names=F,quote=F)
close(con4)

con5 <- gzfile(paste0(params$outdir,"/h2_topline_sex_ns_temp.tsv.gz"),"w")
write.table(sex_ns,file=con5,sep='\t',col.names=T,row.names=F,quote=F)
close(con5)

con6 <- gzfile(paste0(params$outdir,"/h2_topline_onesex_ns_temp.tsv.gz"),"w")
